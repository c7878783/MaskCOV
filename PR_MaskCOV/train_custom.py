...
if __name__ == '__main__':
    ...
    print("parameters = {0} M".format(num_param/1e6))
    # load model
    ...
    if args.lr_scheduler == 'stage':
        ...
        train(Config,#代变化
            ...
            savepoint=args.save_point,
            checkpoint=args.check_point)
    elif args.lr_scheduler == 'cos_step':
        print('cosine_step学习率', flush=True)
        exp_lr_scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epoch * len(dataloader['train']), eta_min=0.000001)
        train_cos(Config,#batch变化
            model,
            epoch_num=args.epoch,
            start_epoch=args.start_epoch,
            optimizer=optimizer,
