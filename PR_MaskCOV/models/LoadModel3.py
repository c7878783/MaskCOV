...
import pdb
# 59.17
class MainModel3(nn.Module):
    def __init__(self, config,args):
        super(MainModel3, self).__init__()
        ...
        if self.backbone_arch == 'resnet18':
            ...
            self.conv_dim = nn.Conv2d(512, 6, kernel_size=1, stride=1, padding=0, bias=False)
            ...
            if self.backbone_arch == 'se_resnet101':
                self.model = nn.Sequential(*list(self.model.children())[:-2])
            self.conv_dim = nn.Conv2d(2048, 6, kernel_size=1, stride=1, padding=0, bias=False)
            self.avgpool = nn.AdaptiveAvgPool2d(output_size=1)
    ...
    def cal_covariance(self, input):
        ...
        for i in range(l):
            ...
            covariance_matrices_list.append(covariance_matrix)
        
        # 将所有图像的协方巧矩阵堆叠成一个张量
        return torch.stack(covariance_matrices_list)  # 最终形状: [6, 6, 6]
